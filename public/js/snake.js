...
function juego() {
  ...
  if (juegoTerminado) {
    ...
    // NUEVO: al terminar la partida guardamos el puntaje en la BD
    guardarPuntaje(JUEGO_ID, puntaje);
    return;
  }
...
document.addEventListener("keydown", function (evento) {
  ...
  if (evento.key === "ArrowUp" && !yendoAbajo)      { direccionX = 0; direccionY = -tamanoCuadro; }
  if (evento.key === "ArrowDown" && !yendoArriba)   { direccionX = 0; direccionY = tamanoCuadro; }
  if (evento.key === "ArrowLeft" && !yendoDerecha)  { direccionX = -tamanoCuadro; direccionY = 0; }
  if (evento.key === "ArrowRight" && !yendoIzquierda){ direccionX = tamanoCuadro; direccionY = 0; }
});
...
canvas.addEventListener("touchend", function (evento) {
  ...
  if (Math.abs(difX) > Math.abs(difY)) {
    if (difX > 0 && !yendoIzquierda)      { direccionX = tamanoCuadro; direccionY = 0; }
    else if (difX < 0 && !yendoDerecha)   { direccionX = -tamanoCuadro; direccionY = 0; }
  } else {
    if (difY > 0 && !yendoArriba)         { direccionX = 0; direccionY = tamanoCuadro; }
    else if (difY < 0 && !yendoAbajo)     { direccionX = 0; direccionY = -tamanoCuadro; }
  }
});
